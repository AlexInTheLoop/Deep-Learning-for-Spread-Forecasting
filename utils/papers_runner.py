...
class PaperEstimatorsRunner:
    ...
    def get_estimates(self, force=False):
        """Calcule/charge les estimateurs pour toutes les périodes et actifs."""
        ...
        for (y, m), dm in self._dms.items():
            ...
            for sym in self.symbols:
                ...
                for d, grp in df.groupby("day", sort=True):
                    est = compute_parametric_estimators(grp[["open", "high", "low", "close"]],
                                                        use_opposed=self.use_opposed)
    ...
    def _load_labels_all(self):
        ...
        self._df_labels = (
            pd.concat(frames, ignore_index=True).set_index(["symbol", "year", "month", "day"]).sort_index())
